...
A histogram of the distribution of the correlations is shown in Figure \ref{fig:growthcorr} (see Methods section for more details).
...
\end{figure}
From this point on, we focus our analysis on this group of proteins that are strongly positively correlated with growth rate.
...
In order to compare the responses of differnt proteins across conditions, we therefore, for every protein, divided its concentration under every condition by its average concentration across all of the conditions (see \ref{concacrossconds} for further details).
This modified concentration across conditions thus represents the normalized (in the sence of specific-protein concentration independent) response of that protein to the different conditions under which it was measured.
...
We find that the distribution of estimated slopes can result from having a common, single response slope, given the experimental noise in the measurements (For further discussion see SI)\emph{ToDo:1. The red line in the figure, representing the expected distribution given the average slope and noise clearly does not fit the results, more thought is needed to settle this}.
...
We therefore observe that a large fraction \emph{ToDo:make concrete by calculating} of the change in the proteome composition across conditions is the result of scaling with growth rate of many proteins.
...
The actual values obtained for the data analyzed are $\alpha=0.34$ for \parencite{Valgepea2013} and $\alpha=0.44$ for \parencite{Heinemann2014}, correspoinding to protein half lives times of $T_{\text{deg}}=2$ hours and $T_{\text{deg}}=1.6$ hours respectively.
...
Our analysis suggests that, while changes in the proteome composition may seem complex, they can, to a large extent\emph{ToDo: quantify by variation explained}, be attributed to the linear increase with growth rate of a large number of proteins, at the expense of a few, down-regulated proteins.
...
Specifically, for \emph{S.cerevisiae} a few recent studies found that the concentration of the majority of the proteins is coordinated across conditions \parencite{Keren2013a, Gasch2000, Brauer2008a} and increases with growth rate.\emph{ToDo:verify this for these references and add numbers} 
