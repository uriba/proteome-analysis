...
A histogram of the distribution of the correlations is shown in Figure \ref{fig:growthcorr} (ToDo:ref Methods).
We find that $\approx 1/3$ of the proteins ($724$ out of $1654$ measured in the data set from [ref Heinemann], hereafter referred to as H, and $296$ out of $919$ in the data set from \parencite{Valgepea2013}, hereafter referred to as V) are strongly positively correlated with the growth rate, where a strong correlation with growth rate was defined as a correlation of $R\geq 0.8$ for the V data set, and $R\geq 0.4$ for the H data set (\emph{ToDo: see SI for further discussion on thresholds}).
...
The concentration of a specific protein in the biomass can be calculated given this fraction and the concentration of total protein in the biomass, which is known to be relatively constant \parencite{eco-sal} (for further discussion see \ref{protconc}).
...
We denote the affinity of gene $i$ under growth condition $c$ by $w_i(c)$ (the notion of affinity for expression is not new, and was first suggested in  \parencite{Maaloe1969}).
...
The actual values obtained for the data analyzed are $\alpha=0.34$ for \parencite{Valgepea2013} and $\alpha=0.44$ for \parencite{Heinemann2014}, corresponding to protein half lives times of $T_{\text{deg}}=2$ hours and $T_{\text{deg}}=1.6$ hours respectively.
