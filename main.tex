...
\end{figure}
From this point on, we focus our analysis on this group of significantly positivly correlated with growth rate proteins (as defined above).
...
This scaling with growth rate is proportional, maintaing the proteins relative abundances, and encompases many proteins from different functional groups and cellular mechanisms.
...
Thus, accounting for protein degradation may serve as a partial explanation for the discrepency between experimental data and the model's predictions.
...
This assumption fails if the rate at which the bio-synthetic machinary operates changes across conditions.
...
(ToDo - elaborate and include regorous definitions as well as experimental evidence, to the extent that such evidence exists)
\section{Discussion}
...
The framework we present emphesizes the importance of accounting for global factors (that are reflected in the growth rate) when analyzing gene expression and proteomics data.
...
Interestingsly, our model demonstrates that no specific control mechanisms need to exist in order to achieve a linear correlation between ribosomal proteins and the growth rate.
...
A discussion for various reasons that may underly these differences is in \ref{ribosomeconc}.
...
\subsubsection{Discussion of reasons for differring ribosome concentration relation to growth rate}
\label{ribosomeconc}
