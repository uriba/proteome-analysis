...
My intermediate results include an extensive analysis of two proteomics data sets collected for the model organism \emph{E.coli} under various environmental conditions.
This analysis focuses on changes in the proteome that are correlated with the growth rate.
The main finding of the analysis is that the concentrations of between a third to a half of the proteins increase with the growth rate in a relatively coordinated manner.
As part of the research we developed a model that explains why the observed phenomena can result from basic physiological considerations, under some trivial assumptions.
...
Following the intermediate results, I present several future directions for my research.
Some of these directions rely on a turbidostat/chemostat system that is currently being built in our lab, a system that can be used to collect further data on physiological changes and the proteome composition under different growth rates and nutrient types.
}
\clearpage        
