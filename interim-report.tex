...
My intermediate results present an extensive analysis of two proteomics data sets collected for the model organism \emph{E.coli} under various environmental conditions.
This analysis focuses on changes in the proteome that are correlated with the growth rate.
The main finding of the analysis is that the concentrations of between a third to a half of the proteins increase with the growth rate.
I present a model that explains why the observed phenomena can result from basic physiological considerations, under some trivial assumptions.
However, despite the magnitude of this phenomena, it only accounts for $\approx 10 \%$ of the variability observed in the proteome in these data sets.

Our lab currently builds a turbidostat/chemostat system can be used to collect further data on physiological changes and the proteome composition under different growth rates and nutrient types.
This data can be used both to strengthen existing results as well as to investigate other phenomena as described in the future plans section.
}
\clearpage        
...
Following are short discussions of several directions I consider pursuing next in my research with some background on each.
...
An alternative assessment method, ribosome profiling, is also reported to have similar noise of $\approx 1.3$ fold \cite{Li2014}.
...
It is thus unclear to what extent conducting such measurements will increase our ability to analyze whole proteome trends.
...
\begin{eqnarray}
    ...
    P=RT_d\alpha
\end{eqnarray}
Where the first relation states the mass ratio between ribosomal RNA, $R_r$, and ribosomal protein, $R_p$, per cell, is 2, the second equation states that the total ribosomal mass in the cell is the sum of the ribosomal RNA and ribosomal protein, and the third equation states that the entire protein mass of the cell, $P$, must be produced by the ribosomes in the cell during one doubling time, $T_d$, at a translation rate $\alpha$ (in units of protein mass synthesized per ribosomal mass per unit time.
...
Examples of such tasks-mechanisms pairs are the production of the proteome of the cell by ribosomes, production of RNA by RNA polymerase, replication of DNA by DNA polymerase, etc. but also other, non trivial tasks such as protein folding by protein-folding proteins and energy consumption by the different cellular processes.
A set of equations can then be written that relates the magnitude of each cellular task (determined by summing over the demands for that task by each cellular component) to the amount of cellular component that performs that task times its rate of operation, times the doubling time.
...
\subsection{Model expected effects of different ribosomoal characteristics on ribosome concentration and use existing data to assess their significance}
...
For example, translation rate can be assumed to change between 10 to 20 amino acids per second per ribosome as a function of the growth rate.
