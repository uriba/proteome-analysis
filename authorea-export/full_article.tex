...
Taken together, these studies suggest that the expression of all genes changes with growth rate, with different factors and architectures of regulatory networks yielding differences in the direction and magnitude of these changes \cite{Klumpp2009}. 
...
A strong positive Pearson correlation between the fraction out of the proteome and the growth rate is observed for a large number of proteins in the two data sets analyzed (upper panels).
Thresholds used for high correlation are marked in dashed lines.
Functional groups are denoted by different colors.
Ribosomal genes are considered under ``genetic information processing''.
Shuffling the amounts of every protein across conditions in the two data sets reveals the bias towards positive correlation with growth rate is a non-trivial feature of the data as is demonstrated in the lower panels.
%
}
\end{center}
\end{figure}
