...
%regoriously define w_i(c) based on gibbs free energies
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% to do prior to submission:
%correlation graph between the proteins of the two data sets analyzed - avg prot frac vs avg prot frac, correlation vs correlation, consider using only glucose chemostats or all conditions, or both. 
%relate to klumpp paper, point out number of proteins measured, consider relating to specific data they use.
...
These data sets were measured under conditions that make them less adequate for our analysis as is further discussed in \ref{furtherdatasetanalysis}.
...
The expected distribution is shown in gray line in Fig.~\ref{fig:globalfit} (Further details on the calculation as well as the deviation in maximum between the expected and observed distributions are discussed in \ref{slopedistexp}).
...
Nonetheless, many such mechanisms have been shown to exist \cite{Nomura1984,Scott2014} (for a comparison to the model suggested in \cite{Scott2014} see \ref{scottcompare}).
...
\subsubsection{Filtering out conditions from the data set from \cite{Schmidt2015}}
\label{heinemanncond} 
...
The predicted fraction of an unregulated protein as a function of the growth rate is to follow a linear trend crossing at the origin (Fig.~\ref{fig:theoreticalpred}, blue dots).
Degradation can be interpreted as implying that the observed growth rate is the combination of the bio-synthesis rate minus the degradation rate, implying that the predicted fraction of an unregulated protein is linear increase with growth rate, but with a horizontal intercept at minus the degradation rate as is shown by the green dots in Fig.~\ref{fig:theoreticalpred}.
...
\end{equation}
where $g(c)$ is the growth rate, $\eta(g(c))$ is the bio-synthesis rate at growth rate $g(c)$, $\eta_0$ is the maximal bio-synthesis rate and $g_m$ the growth rate at which the bio-synthesis rate is $\frac{1}{2}$ the maximal rate (Fig.~\ref{fig:theoreticalpred}, red line).
...
\end{equation}
Surprisingly, this equation also describes the fraction as being linearly dependent on the growth rate, with the kinetic parameters implying a non-zero fraction at zero growth rate as is shown by the red dots in Fig.~\ref{fig:theoreticalpred}.
...
\subsection{Databases omitted from the main analysis}
...
\subsection{Correlation between the different proteomics data sets}
...
Fig.~\ref{fig:chemcorrcomp} shows the relations between all of these data sets.
...
Fig.~\ref{fig:threshold} shows how the choice of threshold affects the fraction of explained variability in the proteome by the linear dependence on growth rate of the proteins that have a correlation with growth rate that is higher than the threshold (blue line).
...
As can be seen in Fig.~\ref{fig:threshold}, our choice of threshold of $0.5$ is relatively close to the optimum value that is $0.25$ for the data set from \cite{Schmidt2015}, and $0.2$ for the data set from \cite{Peebo_2015}.
Moreover, as Fig.~\ref{fig:threshold} illustrates, the different plotted statistics do not change markedly due to this sub-optimal choice of threshold and thus this choice does not affect our results significantly.
...
\subsection{Calculation of expected distribution given experimental noise and deviation of maximum between between the expected distribution and the observed one}
\label{slopedistexp}
...
Fig.~\ref{fig:LB} shows the implications of including the fast growth conditions in the analysis.
...
\subsection{Comparison of the passive resource allocation model to the sectorial model from \cite{Scott2014}}
\label{scottcompare}
