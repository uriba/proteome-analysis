...
%%%%%
%%\usepackage[sorting=none,firstinits=true,isbn=false,url=false,doi=false,eprint=false,maxbibnames=4]{biblatex}
%%\renewbibmacro{in:}{}
...
A fundamental system biology challenge is to understand and predict changes in gene expression levels. In the last two decades, with the ability to measure genome-wide expression levels, there is an avalanche of new data and a need for principles and models to facilitate their interpretation. Early on it was found that ribosome concentration increases in proportion to the specific growth rate \cite{Schaechter1958}.
...
More recently, it was found that changes in gene expression as a function of growth rate are not limited to ribosomal genes.
...
\subsection{Incorporation of synthesis and degradation rates into the model accounts for biosynthetic activity at zero growth rate}
...
In all subsequent analysis we have omitted four outlier conditions out of the $\tothConds$ in \cite{Heinemann2015} as discussed in \cite{Barenholz2015}.
...
These values are much higher than those obtained for randomized data sets (\hGlobalShuff{} and \vGlobalShuff{} strongly positively correlated proteins for the two data sets, respectively, when the amounts of each protein are shuffled across conditions, Figure \ref{fig:growthcorr}C).
...
For the data from \cite{Peebo_2015} the expected distribution should be narrower \cite{Barenholz2015}.
...
This change is smaller than the $1:1$ change predicted by our basic model and the deviations may partly result from the effects of degradation and varying bio-synthesis rates discussed above.
