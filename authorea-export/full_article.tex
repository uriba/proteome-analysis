...
%db correlation plots and data,
%grcorr comparison between gradual titrated databases.
...
% relate to hwa/scott paper and their sectorial model vs ours.
...
We compare and take ratios of intrinsic affinites to show how they can be used to understand changes in specific growth rate as a result of expression modulation.
Conceptually, intrinsic affinities resemble standard Gibbs energies, they integrate many parameters, are given under some standard condition, represent tendencies of processes to to occur, and can be used in different contexts without knowing their exact values.
...
Experimental evindece in \emph{S.cerevisiae} suggest that protein degradation occures at relatively slow rates with half lives of $\approx10$ hours \cite{Christiano2014}.
...
While ribosomal translation rates and mRNA transcription rates are relatively constant per synthesizeing machinary unit, they may change up to 2 fold across different conditions \cite{Bremer1987}.
...
These data sets where measured under conditions that make them less adequate for our analysis as is further discussed in \ref{furtherdatasetanalysis}.
...
A few reasons may explain why switching between discrete regulatory states occures when gradually modulating growth rate and how discrete regulatory mechanisms may yield continuesly modified observed affinities: 
...
Such modulations yield very typical distributions of correlation with growth rate of proteins roughly dividing the proteome to 3 groups, proteins the concentration of which increases with growth rate, proteins the concentration of which decreases with growth rate, and proteins that show no evident correlation with growth rate \ref{fig:grcorr_dbs}.

Under such conditions it is evident that one cannot distinguish between proteins that are actively regulated, and proteins that are passively regulated, as in either case (by our predictions) such proteins will increase in concentration with growth rate.
Furthermore, smaller number of conditions increases the probabilty of proteins to yield extremely high or low correlation with growth rate due to measurement noise.
To illustrate, in the extreme case of measureing only two conditions, every protein (unless presenting exactly the same amount under the two conditions) will have either a perfect positive correlation with the growth rate, or a perfect negative correlation with the growth rate, depending on whether it is higherly expressed under the second condition or the first one.
...
To assess the agreement between the different data sets we used, as well as those we ommitted, we compared the correlation with growth rate of each protein under similar conditions between every two of these data sets.
...
In the data set from \cite{Hui_2015} we used each limiting set seperately.
...
Finally, the tranlation limiting conditions from \cite{Hui_2015} present a small negative covariance with all of the other conditions.
...
This analysis emphesizes the importance of analyzing each data set seperately as consistency between data sets from different labs is not very high.
...
According to this model, the proteome is roughly divided to sectors with the main sectors being a $Q$ sector that is unaffected by growth rate, a $R$ sector composed of ribosomal proteins and translation related proteins, and a $P$ section composed of metabolic proteins including catabolic and anabolic enzymes (the $P$ section can be further sub-divided according to the specific metabolic tasks groups of proteins perform).
In this model, a specific protein may belong to more than one section, where, for example, if protein $P_i$ has a minimal value $P_i^\min$, then that fraction can be accounted for in section $Q$, and only the changing fraction will be accounted for in section $R$ or section $P$, according to $P_i$'s function.
The main point of the model is that different triggers may increase or decrease the expression of entire sections, thus serving as global, or master, regulators.
The model thus allows one to get an overview of the entire proteome as a result of the levels of a few master regulators.
