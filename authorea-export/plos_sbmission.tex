...
Studies examining the interplay between global and specific modes of regulation, suggested that global factors play a major role in determining the expression levels of genes \cite{Gasch2000,Klumpp2009,Klumpp2014,Scott2010,Scott2014,Berthoumieux2013,Keren2013,Gerosa2013,Valgepea2013,Hui_2015}.
...
\subsection*{Normalizing protein fractions across conditions}
...
\subsection*{Filtering out conditions from the data set from \cite{Schmidt2015}}
\label{heinemanncond} 
...
\paragraph{The expression level of a protein can be decomposed into gene specific control and global
...
To determine the resulting fraction of every protein, our model assumes that the bio-synthetic resources are distributed among the genes according to those affinities, as is stated in Eq.~(\ref{eq:concentration-ratio}).
...
As the decrease in total proteome mass per cell dry weight is relatively small, it will not cause a qualitative difference in our findings even when considering specific protein concentration and not protein fraction out of the proteome (for further discussion see \ref{protconc}).
...
\paragraph{Growth rate is the combined outcome of proteome composition and environmental conditions}
...
We incorporate the effects of changes to synthesis rates into our model in section \ref{slowbiosynthesis}.
...
\end{equation}
This equation predicts linear dependence of the fraction of unregulated proteins on the growth rate, with an intercept with the horizontal axis occurring at minus the degradation rate (Fig.~\ref{fig:theoreticalpred}).
...
Thus, lower synthesis rates under slower growth rates will be reflected by a lower slope and higher interception point for non-regulated proteins than those predicted by the constant-rate version of the model, as is depicted in Fig.~\ref{fig:theoreticalpred}.
...
{\bf Histogram of the slopes of regression lines for the highly correlated with growth proteins (\hGlobal{} and \vGlobal{} proteins in the left and right panels respectively).}
    ...
    Ribosomal proteins, shown in green, scale with growth rate in a manner similar to the rest of the high correlation proteins (see text and Fig.~\ref{fig:ribsnonribs}).
%
}
\end{center}
\end{figure}
...
Despite the magnitude of this phenomena, the fraction of the total variability in the proteome that is accounted for by this linear response is only $\approx 8\%$  in the data set from \cite{Schmidt2015} and even lower in \cite{Peebo_2015} (Fig.~\ref{fig:threshold}).
...
Finally, the fraction of variability in the proteome that can be explained by a single linear regression to these proteins is smaller for the shuffled data sets than that obtained for the real data set ($2\%$ vs. $8\%$ for a threshold of $R\ge0.5$ for the data from \cite{Schmidt2015} and $1\%$ vs. $3.5\%$ for the data set from \cite{Peebo_2015}), as is seen in Fig.~\ref{fig:shuffledexpvar}.

We find that the simulated (second) set does display similar characteristics to those we find in the real data, confirming that if, indeed, our model is valid, experimental measurements would overlap with those that we obtained as is seen in Fig.~\ref{fig:simulated}.
...
The framework we present emphasizes the importance of accounting for global factors, that are reflected in the growth rate, when analyzing gene expression and proteomics data, as was noted before \cite{Maaloe1969,Brauer2008,Klumpp2009,Klumpp2014,Scott2014,Scott2010,Berthoumieux2013,Keren2013,Gerosa2013,Valgepea2013,Hui_2015,Peebo_2015,Wei_e_2015}.
...
Nonetheless, many such mechanisms have been shown to exist \cite{Nomura1984,Scott2014} (for a comparison to the model suggested in \cite{Scott2014} see \nameref{scottcompare}).
...
{\bf Effects of degradation and varying synthesis rate on model predictions}
The predicted fraction of an unregulated protein as a function of the growth rate is to follow a linear trend crossing at the origin (Figure \ref{fig:theoreticalpred}, blue dots).
Degradation can be interpreted as implying that the observed growth rate is the combination of the bio-synthesis rate minus the degradation rate, implying that the predicted fraction of an unregulated protein is linear increase with growth rate, but with a horizontal intercept at minus the degradation rate as is shown by the green dots in Figure \ref{fig:theoreticalpred}.
...
{\bf Databases omitted from the main analysis}
...
\caption{\label{fig:grcorr_dbs}
Histograms of the correlation with growth rate of proteins in data sets obtained by gradually modifying the growth rate with constant input media.
...
\subsection*{S3 text}
\label{datasetscorr}
{\bf Correlation between the different proteomics data sets}
...
Figure \ref{fig:chemcorrcomp} shows the relations between all of these data sets.
...
\caption{ \label{fig:chemcorrcomp}
Pair-wise comparison of the data sets used in this work.
...
\subsection*{S4 text}
\label{corrthreshold} 
{\bf Threshold selection for defining strong correlation with growth rate}
...
\subsection*{S5 text}
\label{slopedistexp}
{\bf Calculation of expected distribution given experimental noise and deviation of maximum between between the expected distribution and the observed one}
...
These average slope and average standard error were used to calculate the expected distribution of slopes that would emerge, should all proteins share the same average slope with the given average standard error in measurements.
...
\subsection*{S6 text}
\label{lbanalysis}
{\bf Analysis of the data set from \cite{Schmidt2015} including fast growth conditions}
...
\subsection*{S7 text}
\label{ribosomeconc} 
{\bf Discussion of reasons for differing ribosome concentration relation to growth rate}
...
\caption{Breakdown by function of strongly positively correlated with growth rate proteins in the data set from \cite{Schmidt2015}%
}
\end{table}
...
\caption{Breakdown by function of strongly positively correlated with growth rate proteins in the data set from \cite{Peebo_2015}%
}
\end{table}

\subsection*{S8 text}
{\bf Ribosomal proteins scale similarly to non-ribosomal proteins that are strongly positively correlated with growth rate}
Comparing the normalized sum of ribosomal proteins to the normalized sum of the positively correlated with growth rate proteins that are non-ribosomal shows that these two groups scale in the same way with the growth rate, as is seen in Fig.~\ref{fig:ribsnonribs}
...
{\bf The scaling with growth rate of ribosomal proteins and non-ribosomal, but highly correlated with growth rate proteins is shown.}
...
Our model therefore aggregates all non-actively regulated proteins between two specific conditions into one group, $A$ and suggests that all of these proteins, despite not being actively regulated, may still occupy different fractions out of the proteome between the two conditions, if the active regulation of the proteins that do get regulated between the two conditions changes their overall expression (noting that proteins that do get actively regulated between two conditions may either be up regulated or down regulated, so their total sum may either increase, decrease, or remain the same between the two conditions observed).

To summarize, where the model from \cite{Scott2014} focuses on what classes of proteins exist, what are the master regulators, and how are they used to achieve optimal growth, the model presented here focuses on global patterns that passively emerge due to specific regulation on specific genes (be it regulation that is targeted at only a small number of genes, or a large group of genes), exposing the fact that positive correlation of protein fraction with growth rate can be an emerging property, not requiring active regulation of the positively correlated proteins.
...
M.~Schaechter, O.~Maaloe, and N.~O. Kjeldgaard, ``Dependency on medium and
  temperature of cell size and chemical composition during balanced growth of
  salmonella typhimurium,'' {\em Microbiology}, vol.~19, no.~3, pp.~592--606,
  1958.

\bibitem{Neidhardt1999}
F.~C. Neidhardt, ``{Bacterial Growth: Constant Obsession with dN/dt},'' {\em J.
  Bacteriol.}, vol.~181, pp.~7405--7408, dec 1999.

\bibitem{Dennis2004}
P.~P. Dennis, M.~Ehrenberg, and H.~Bremer, ``{Control of rRNA Synthesis in
  Escherichia coli : a Systems Biology Approach Control of rRNA Synthesis in
  Escherichia coli : a Systems Biology Approach {\textdagger}},'' vol.~68,
  no.~4, 2004.

\bibitem{Zaslaver2009}
A.~Zaslaver, S.~Kaplan, A.~Bren, A.~Jinich, A.~Mayo, E.~Dekel, U.~Alon, and
  S.~Itzkovitz, ``{Invariant distribution of promoter activities in Escherichia
  coli.},'' {\em PLoS computational biology}, vol.~5, p.~e1000545, oct 2009.

\bibitem{Molenaar2009}
D.~Molenaar, R.~van Berlo, D.~de~Ridder, and B.~Teusink, ``{Shifts in growth
  strategies reflect tradeoffs in cellular economics.},'' {\em Molecular
  systems biology}, vol.~5, p.~323, jan 2009.

\bibitem{Nomura1984}
M.~Nomura, R.~Gourse, and G.~Baughman, ``{REGULATION OF THE SYNTHESIS OF
  RIBOSOMES AND RIBOSOMAL COMPONENTS},'' {\em Annual review of biochemistry},
  pp.~75--117, 1984.

\bibitem{Murray_2003}
H.~D. Murray, D.~A. Schneider, and R.~L. Gourse, ``{Control of {rRNA}
  Expression by Small Molecules Is Dynamic and Nonredundant},'' {\em Molecular
  Cell}, vol.~12, pp.~125--134, jul 2003.
...
E.~Bosdriesz, D.~Molenaar, B.~Teusink, and F.~J. Bruggeman, ``{How fast-growing
  ...
  growth-rate maximization},'' {\em {FEBS} Journal}, pp.~n/a--n/a, mar 2015.

\bibitem{Chatterji2001}
D.~Chatterji and A.~{Kumar Ojha}, ``{Revisiting the stringent response, ppGpp
  and starvation signaling},'' 2001.

\bibitem{Brauer2008a}
M.~J. Brauer, C.~Huttenhower, E.~M. Airoldi, R.~Rosenstein, J.~C. Matese,
  D.~Gresham, V.~M. Boer, O.~G. Troyanskaya, and D.~Botstein, ``{Coordination
  of Growth Rate, Cell Cycle, Stress Response, and Metabolic Activity in
  Yeast},'' 2008.

\bibitem{Saldanha2004}
A.~J. Saldanha, M.~J. Brauer, and D.~Botstein, ``{Nutritional homeostasis in
  batch and steady-state culture of yeast.},'' {\em Molecular biology of the
  cell}, vol.~15, no.~9, pp.~4089--4104, 2004.

\bibitem{You_2013}
C.~You, H.~Okano, S.~Hui, Z.~Zhang, M.~Kim, C.~W. Gunderson, Y.-P. Wang,
  P.~Lenz, D.~Yan, and T.~Hwa, ``{Coordination of bacterial proteome with
  metabolism by cyclic {AMP} signalling},'' {\em Nature}, vol.~500,
  pp.~301--306, aug 2013.

\bibitem{Peebo_2015}
K.~Peebo, K.~Valgepea, A.~Maser, R.~Nahku, K.~Adamberg, and R.~Vilu,
  ``{Proteome reallocation in Escherichia coli with increasing specific growth
  rate},'' {\em Mol. {BioSyst}.}, vol.~11, no.~4, pp.~1184--1193, 2015.

\bibitem{Keren2013}
L.~Keren, O.~Zackay, M.~Lotan-Pompan, U.~Barenholz, E.~Dekel, V.~Sasson,
  G.~Aidelberg, A.~Bren, D.~Zeevi, A.~Weinberger, U.~Alon, R.~Milo, and
  E.~Segal, ``{Promoters maintain their relative activity levels under
  different growth conditions},'' {\em Molecular Systems Biology}, vol.~9,
  p.~701, 2013.
...
M.~J. Brauer, C.~Huttenhower, E.~M. Airoldi, R.~Rosenstein, J.~C. Matese,
  D.~Gresham, V.~M. Boer, O.~G. Troyanskaya, and D.~Botstein, ``{Coordination
  of growth rate, cell cycle, stress response, and metabolic activity in
  yeast.},'' {\em Molecular biology of the cell}, vol.~19, pp.~352--67, jan
  2008.

\bibitem{Castrillo2007}
J.~I. Castrillo, L.~A. Zeef, D.~C. Hoyle, N.~Zhang, A.~Hayes, D.~C.~J. Gardner,
  M.~J. Cornell, J.~Petty, L.~Hakes, L.~Wardleworth, B.~Rash, M.~Brown, W.~B.
  Dunn, D.~Broadhurst, K.~O'Donoghue, S.~S. Hester, T.~P.~J. Dunkley, S.~R.
  Hart, N.~Swainston, P.~Li, S.~J. Gaskell, N.~W. Paton, K.~S. Lilley, D.~B.
  Kell, and S.~G. Oliver, ``{Growth control of the eukaryote cell: a systems
  biology study in yeast.},'' {\em Journal of biology}, vol.~6, no.~2, p.~4,
  2007.

\bibitem{Gerosa2013}
L.~Gerosa, K.~Kochanowski, M.~Heinemann, and U.~Sauer, ``{Dissecting specific
  and global transcriptional regulation of bacterial gene expression.},'' {\em
  Molecular systems biology}, vol.~9, no.~658, p.~658, 2013.

\bibitem{Gasch2000}
A.~P. Gasch, P.~T. Spellman, C.~M. Kao, O.~Carmel-Harel, M.~B. Eisen, G.~Storz,
  D.~Botstein, and P.~O. Brown, ``{Genomic expression programs in the response
  of yeast cells to environmental changes.},'' {\em Molecular biology of the
  cell}, vol.~11, no.~12, pp.~4241--4257, 2000.

\bibitem{Klumpp2009}
S.~Klumpp, Z.~Zhang, and T.~Hwa, ``{Growth rate-dependent global effects on
  gene expression in bacteria.},'' {\em Cell}, vol.~139, pp.~1366--75, dec
  2009.

\bibitem{Klumpp2014}
S.~Klumpp and T.~Hwa, ``{Bacterial growth: global effects on gene expression,
  growth feedback and proteome partition},'' {\em Current Opinion in
  Biotechnology}, vol.~28, pp.~96--102, 2014.

\bibitem{Scott2010}
M.~Scott, C.~W. Gunderson, E.~M. Mateescu, Z.~Zhang, and T.~Hwa,
  ``{Interdependence of cell growth and gene expression: origins and
  consequences.},'' {\em Science (New York, N.Y.)}, vol.~330, pp.~1099--102,
  nov 2010.

\bibitem{Scott2014}
M.~Scott, S.~Klumpp, E.~M. Mateescu, and T.~Hwa, ``{Emergence of robust growth
  laws from optimal regulation of ribosome synthesis.},'' {\em Molecular
  systems biology}, vol.~10, p.~747, jan 2014.

\bibitem{Berthoumieux2013}
S.~Berthoumieux, H.~de~Jong, G.~Baptist, C.~Pinel, C.~Ranquet, D.~Ropers, and
  J.~Geiselmann, ``{Shared control of gene expression in bacteria by
  transcription factors and global physiology of the cell.},'' {\em Molecular
  systems biology}, vol.~9, p.~634, jan 2013.

\bibitem{Valgepea2013}
K.~Valgepea, K.~Adamberg, A.~Seiman, and R.~Vilu, ``{Escherichia coli achieves
  faster growth by increasing catalytic and translation rates of proteins.},''
  {\em Molecular BioSystems}, vol.~9, pp.~2344--58, sep 2013.

\bibitem{Hui_2015}
S.~Hui, J.~M. Silverman, S.~S. Chen, D.~W. Erickson, M.~Basan, J.~Wang, T.~Hwa,
  and J.~R. Williamson, ``{Quantitative proteomic analysis reveals a simple
  strategy of global resource allocation in bacteria},'' {\em Molecular Systems
  Biology}, vol.~11, pp.~e784--e784, feb 2015.

\bibitem{Klumpp2008}
S.~Klumpp and T.~Hwa, ``{Growth-rate-dependent partitioning of RNA polymerases
  in bacteria.},'' {\em Proceedings of the National Academy of Sciences of the
  United States of America}, vol.~105, pp.~20245--50, dec 2008.

\bibitem{Regenberg2006}
B.~Regenberg, T.~Grotkjaer, O.~Winther, A.~Fausb\o~ll, M.~Akesson, C.~Bro,
  L.~K. Hansen, S.~r. Brunak, and J.~Nielsen, ``{Growth-rate regulated genes
  have profound impact on interpretation of transcriptome profiling in
  Saccharomyces cerevisiae.},'' {\em Genome biology}, vol.~7, no.~11, p.~R107,
  2006.

\bibitem{Schmidt2015}
A.~Schmidt, K.~Kochanowski, S.~Vedelaar, E.~Ahrne, B.~Volkmer, L.~Callipo,
  K.~Knoops, M.~Bauer, R.~Aebersold, and M.~Heinemann, ``{The quantitative and
  condition-dependent Escherichia coli proteome},'' {\em Nat Biotech},
  vol.~advance on, dec 2015.

\bibitem{Maaloe1969}
O.~Maaloe, ``{An analysis of bacterial growth},'' {\em Dev Biol Suppl}, vol.~3,
  pp.~33--58, 1969.

\bibitem{Bremer1987}
H.~Bremer and P.~P. Dennis, ``{Modulation of chemical composition and other
  parameters of the cell by growth rate},'' 1996.

\bibitem{24656150}
Z.~Li, M.~Nimtz, and U.~Rinas, ``{The metabolic potential of Escherichia coli
  BL21 in defined and rich medium.},'' {\em Microb Cell Fact}, vol.~13, p.~45,
  Mar 2014.

\bibitem{10515934}
H.~Tao, C.~Bausch, C.~Richmond, F.~Blattner, and T.~Conway, ``{Functional
  genomics: expression analysis of Escherichia coli growing on minimal and rich
  media.},'' {\em J Bacteriol}, vol.~181, pp.~6425--40.

\bibitem{ingraham1983growth}
J.~L. Ingraham, O.~Maal\o~e, and F.~C. Neidhardt, {\em {Growth of the bacterial
  cell}}.
\newblock Sinauer Associates, 1983.

\bibitem{Pedersen1978a}
S.~Pedersen, P.~L. Bloch, S.~Reeh, and F.~C. Neidhardt, ``{Patterns of protein
  synthesis in E. coli: a catalog of the amount of 140 individual proteins at
  different growth rates.},'' {\em Cell}, vol.~14, no.~1, pp.~179--190, 1978.

\bibitem{Christiano2014}
R.~Christiano, N.~Nagaraj, F.~Fr{\"{o}}hlich, and T.~C. Walther, ``{Global
  Proteome Turnover Analyses of the Yeasts S. cerevisiae and S. pombe.},'' {\em
  Cell reports}, pp.~1959--1965, 2014.

\bibitem{Paalme_1995}
T.~Paalme, A.~Kahru, R.~Elken, K.~Vanatalu, K.~Tiisma, and V.~Raivo, ``{The
  computer-controlled continuous culture of Escherichia coli with smooth change
  of dilution rate (A-stat)},'' {\em Journal of Microbiological Methods},
  vol.~24, pp.~145--153, dec 1995.

\bibitem{23203884}
H.~Salgado, M.~Peralta-Gil, S.~Gama-Castro, A.~Santos-Zavaleta,
  L.~Mu{\~{n}}iz-Rascado, J.~Garc{\'{\i}}a-Sotelo, V.~Weiss, H.~Solano-Lira,
  I.~Mart{\'{\i}}nez-Flores, A.~Medina-Rivera, G.~Salgado-Osorio,
  S.~Alquicira-Hern{\'{a}}ndez, K.~Alquicira-Hern{\'{a}}ndez,
  A.~L{\'{o}}pez-Fuentes, L.~Porr{\'{o}}n-Sotelo, A.~Huerta,
  C.~Bonavides-Mart{\'{\i}}nez, Y.~Balderas-Mart{\'{\i}}nez, L.~Pannier,
  M.~Olvera, A.~Labastida, V.~Jim{\'{e}}nez-Jacinto, L.~Vega-Alvarado, M.-C.~V.
  Del, A.~Hern{\'{a}}ndez-Alvarez, E.~Morett, and J.~Collado-Vides,
  ``{RegulonDB v8.0: omics data sets, evolutionary conservation, regulatory
  phrases, cross-validated gold standards and more.},'' {\em Nucleic Acids
  Res}, vol.~41, pp.~D203--13.

\bibitem{Kaneko2014}
K.~Kaneko, C.~Furusawa, and T.~Yomo, ``{Universal relationship in
  gene-expression changes for cells in steady-growth state},'' p.~7, jul 2014.

\bibitem{Wei_e_2015}
A.~Y. Wei{\ss}e, D.~A. Oyarz{\'{u}}n, V.~Danos, and P.~S. Swain, ``{Mechanistic
  links between cellular trade-offs gene expression, and growth},'' {\em Proc
  Natl Acad Sci {USA}}, p.~201416533, feb 2015.

\bibitem{Srivatsan2008}
A.~Srivatsan and J.~D. Wang, ``{Control of bacterial transcription, translation
  and replication by (p)ppGpp.},'' {\em Current opinion in microbiology},
  vol.~11, no.~2, pp.~100--5, 2008.

\bibitem{Oliphant2007}
T.~E. Oliphant, ``{SciPy: Open source scientific tools for Python},'' {\em
  Computing in Science and Engineering}, vol.~9, pp.~10--20, 2007.

\bibitem{Community2011}
N.~Community, ``{NumPy Reference},'' {\em October}, vol.~1, no.~October,
  pp.~1--1146, 2011.

\bibitem{McKinney2011}
W.~McKinney, ``{pandas: a Foundational Python Library for Data Analysis and
  Statistics},'' in {\em Python for High Performance and Scientific Computing},
  pp.~1--9, 2011.
